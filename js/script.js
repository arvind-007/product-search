    ...
    products = {
        ...
        getSearchSuggetions: (request, response) => {
            $.ajax({
                ...
                dataType: "json",
                success: function (res) {
                    response(res.data.suggest.suggestions[0].options.map((itm) => {
                        return itm.text;
                    }));
                },
                error: function (errMsg) {
            ...
            data.hits.hits.map(p => {
                ...
                <div class="col-lg-4 col-md-6 col-sm-6" id="${p._id}">
                    <div class="product__item">
                        <div class="product__item__pic set-bg" data-setbg="${src.imagecode ? src.imagecode : 'img/product/default.jpg'}"></div>
        ...
        renderRefineByFilters(data) {
            ...
            data.aggregations.attributes.key.buckets.map(f => {
                ...
                $("#product-filter").append(`<div class="sidebar__item"><h4>${f.key} </h4><ul>${filters}</ul></div>`);
            });
        },
        createAggregation() {
            return [{
