$(function () {
  ...
  function getSearchSuggetions1(request, response) {
    $.ajax({
      ...
      success: function (res) {
        response(
          res.data.suggest.suggestions[0]
  ...
  function getSearchSuggetions2(request, response) {
    $.ajax({
      ...
      beforeSend: function (request) {
        request.setRequestHeader(
          "access_token",
          sessionStorage.getItem("ES_ACCESS_TOKEN1")
        );
      },
      url: ES_API_URL + "raw",
      data: JSON.stringify({
  ...
  function getSearchSuggetions4(request, response) {
    $.ajax({
      ...
      beforeSend: function (request) {
        request.setRequestHeader(
          "access_token",
          sessionStorage.getItem("ES_ACCESS_TOKEN2")
        );
      },
      url: ES_API_URL + "raw",
      data: JSON.stringify({
        size: 10,
        _source: ["search_term", "rank"],
        query: {
          bool: {
            should: [
              {
                multi_match: {
                  query: request.term,
                  type: "bool_prefix",
                  fields: ["search_term.lowered^100"],
                },
              },
              {
                function_score: {
